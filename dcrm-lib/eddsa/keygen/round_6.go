...
func (round *round6) Start() error {
	...
	for k,id := range ids {
	    ...
	    if !shareUFlag {
		    ...
		    return errors.New("dcrm back-end internal error:VSS Share verification fail")
	    }
	...
	for k, id := range ids {
		...
		if !bytes.Equal(askBBytes[:], tt[:]) {
			...
			return errors.New("dcrm back-end internal error:VSS Coefficient verification fail")
		}
