...
func (round *round6) Start() error {
	...
	for k, _ := range round.idsign {
	    ...
	    if !CSBFlag {
		    ...
		    return errors.New("dcrm back-end internal error:commitment(CSB) not pass")
	    }
	...
	if err != nil {
	    return errors.New("dcrm back-end internal error:write final r fail in caling k2")
	}

	_,err = h.Write(round.temp.pkfinal[:])
	if err != nil {
	    return errors.New("dcrm back-end internal error:write final pk fail in calling k2")
	}

	_,err = h.Write(([]byte(round.temp.message))[:])
	if err != nil {
	    return errors.New("dcrm back-end internal error:write message fail in caling k2") 
	}
