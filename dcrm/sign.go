...
func EdSignProcessInboundMessages(msgprex string,finishChan chan struct{},wg *sync.WaitGroup,ch chan interface{}) {
    ...
    w, err := FindWorker(msgprex)
    if w == nil || err != nil {
	res := RpcDcrmRes{Ret: "", Err: fmt.Errorf("fail to ed sign process inbound messages")}
	ch <- res
	return 
    }
    ...
    for {
		...
		_,err = w.DNode.Update(mm)
		if err != nil {
		    fmt.Printf("========== EdSignProcessInboundMessages, dnode update fail, receiv dcrm msg = %v, err = %v ============\n",m,err)
		    res := RpcDcrmRes{Ret: "", Err: err}
		    ch <- res
		    return
		}
...
func EdSignGetRealMessage(msg map[string]string) dcrmlib.Message {
    from := msg["FromID"]
    var to []string
    v,ok := msg["ToID"]
    if ok && v != "" {
...
func processSign_ed(msgprex string,msgtoenode map[string]string,errChan chan struct{},outCh <-chan dcrmlib.Message,endCh <-chan edsigning.EdSignData) (*edsigning.EdSignData,error) {
	for {
		select {
		case <-errChan: // when keyGenParty return
		    fmt.Printf("=========== processSign_ed,error channel closed fail to start local dcrm node ===========\n")
			return nil,errors.New("error channel closed fail to start local dcrm node")
