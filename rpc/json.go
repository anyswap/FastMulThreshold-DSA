...
func (c *jsonCodec) ReadRequestHeaders() ([]rpcRequest, bool, Error) {
	...
	if err := c.decode(&incomingMsg); err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,jsonCodec.ReadRequestHeaders,err =%v!!!!===========================================", err)
...
func checkReqId(reqId json.RawMessage) error {
	if len(reqId) == 0 {
		//		fmt.Println("================================!!!smpcwalletrpclog,checkReqId,err =%v!!!!===========================================",fmt.Errorf("missing request id"))
		return fmt.Errorf("missing request id")
	}
	if _, err := strconv.ParseFloat(string(reqId), 64); err == nil {
		//		fmt.Println("================================!!!smpcwalletrpclog,checkReqId,err =%v!!!!===========================================",err)
		return nil
	}
	var str string
	...
	//	fmt.Println("================================!!!smpcwalletrpclog,checkReqId,err =%v!!!!===========================================",fmt.Errorf("invalid request id"))
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	if err := json.Unmarshal(incomingMsg, &in); err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,parseRequest,err =%v!!!!===========================================", err)
	...
	if strings.HasSuffix(in.Method, subscribeMethodSuffix) {
		...
		fmt.Println("================================!!!smpcwalletrpclog,parseRequest,err =%v!!!!===========================================", fmt.Errorf("Unable to parse subscription request"))
		return nil, false, &invalidRequestError{"Unable to parse subscription request"}
	}
...
func parseBatchRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	var in []jsonRequest
	if err := json.Unmarshal(incomingMsg, &in); err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,parseBatchRequest,err =%v!!!!===========================================", err)
		return nil, false, &invalidMessageError{err.Error()}
	}
	...
	for i, r := range in {
		...
		if strings.HasSuffix(r.Method, subscribeMethodSuffix) {
			...
			fmt.Println("================================!!!smpcwalletrpclog,parseBatchRequest,err =%v!!!!===========================================", fmt.Errorf("Unable to parse (un)subscribe request arguments"))
...
func parsePositionalArguments(rawArgs json.RawMessage, types []reflect.Type) ([]reflect.Value, Error) {
	...
	if tok, _ := dec.Token(); tok != json.Delim('[') {
		fmt.Println("================================!!!smpcwalletrpclog,parsePositionalArguments,err =%v!!!!===========================================", fmt.Errorf("non-array args"))
	...
	for i := 0; dec.More(); i++ {
		if i >= len(types) {
			fmt.Println("================================!!!smpcwalletrpclog,parsePositionalArguments,err =%v!!!!===========================================", fmt.Errorf("too many arguments, want at most"))
			return nil, &invalidParamsError{fmt.Sprintf("too many arguments, want at most %d", len(types))}
		}
		argval := reflect.New(types[i])
		if err := dec.Decode(argval.Interface()); err != nil {
			fmt.Println("================================!!!smpcwalletrpclog,parsePositionalArguments,err =%v!!!!===========================================", fmt.Errorf("invalid argument"))
			return nil, &invalidParamsError{fmt.Sprintf("invalid argument %d: %v", i, err)}
		}
		if argval.IsNil() && types[i].Kind() != reflect.Ptr {
			fmt.Println("================================!!!smpcwalletrpclog,parsePositionalArguments,err =%v!!!!===========================================", fmt.Errorf("missing value for required argument"))
	...
	if _, err := dec.Token(); err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,parsePositionalArguments,err =%v!!!!===========================================", err)
...
func (c *jsonCodec) CreateErrorResponse(id interface{}, err Error) interface{} {
	fmt.Println("================================!!!smpcwalletrpclog,jsonCodec.CreateErrorResponse,err =%v!!!!===========================================", err)
...
func (c *jsonCodec) CreateErrorResponseWithInfo(id interface{}, err Error, info interface{}) interface{} {
	fmt.Println("================================!!!smpcwalletrpclog,jsonCodec.CreateErrorResponseWithInfo,err =%v!!!!===========================================", err)
