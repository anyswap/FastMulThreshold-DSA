...
func (s *Server) handle(ctx context.Context, codec ServerCodec, req *serverRequest) (interface{}, func()) {
	if req.err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,Server.handle,err =%v!!!!===========================================", req.err)
	...
	if req.isUnsubscribe { // cancel subscription, first param must be the subscription id
		if len(req.args) >= 1 && req.args[0].Kind() == reflect.String {
			...
			if err := notifier.unsubscribe(subid); err != nil {
				fmt.Println("================================!!!smpcwalletrpclog,Server.handle,err =%v!!!!===========================================", err)
...
func (s *Server) exec(ctx context.Context, codec ServerCodec, req *serverRequest) {
	...
	var callback func()
	if req.err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,Server.exec,err =%v!!!!===========================================", req.err)
...
func (s *Server) execBatch(ctx context.Context, codec ServerCodec, requests []*serverRequest) {
	...
	for i, req := range requests {
		if req.err != nil {
			fmt.Println("================================!!!smpcwalletrpclog,Server.execBatch,err =%v!!!!===========================================", req.err)
	...
	if err := codec.Write(responses); err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,Server.execBatch,22222,err =%v!!!!===========================================", err)
		codec.Close()
	}
...
func (s *Server) readRequest(codec ServerCodec) ([]*serverRequest, bool, Error) {
	...
	if err != nil {
		fmt.Println("================================!!!smpcwalletrpclog,Server.readRequest,err =%v!!!!===========================================", err)
	...
	for i, r := range reqs {
			...
			if callb, ok := svc.subscriptions[r.method]; ok {
				...
				if r.params != nil && len(callb.argTypes) > 0 {
					...
					if args, err := codec.ParseRequestArguments(argTypes, r.params); err == nil {
						...
						fmt.Println("================================!!!smpcwalletrpclog,Server.readRequest,22222,err =%v!!!!===========================================", err)
						requests[i].err = &invalidParamsError{err.Error()}
					}
				}
			} else {
		...
		if callb, ok := svc.callbacks[r.method]; ok { // lookup RPC method
			...
			if r.params != nil && len(callb.argTypes) > 0 {
				if args, err := codec.ParseRequestArguments(callb.argTypes, r.params); err == nil {
					...
					fmt.Println("================================!!!smpcwalletrpclog,Server.readRequest,3333,err =%v!!!!===========================================", err)
					requests[i].err = &invalidParamsError{err.Error()}
				}
			}
			continue
		...
		fmt.Println("================================!!!smpcwalletrpclog,Server.readRequest,44444,err =%v!!!!===========================================", fmt.Errorf("method not found"))
		requests[i] = &serverRequest{id: r.id, err: &methodNotFoundError{r.service, r.method}}
	}
