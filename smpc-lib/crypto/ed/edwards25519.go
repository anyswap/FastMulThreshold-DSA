...
// ** Added and modified by jnxchang, part of content is from https://github.com/HyperspaceApp/ed25519/blob/11c3dc4e75d644d2578563fcd14ad9bf0026abcd/internal/edwards25519/edwards25519.go

// GetBigIntOrder get Order of edwards25519
// order [4]uint64 {0x1000000000000000, 0x0000000000000000, 0x14def9dea2f79cd6, 0x5812631a5cf5d3ed} = 2^252+27742317777372353535851937790883648493
func GetBigIntOrder() *big.Int {
	one := big.NewInt(1)

	order := new(big.Int).Lsh(one, uint(252))
	order = new(big.Int).Add(order, new(big.Int).Lsh(big.NewInt(0x14def9dea2f79cd6), 64))
	order = new(big.Int).Add(order, big.NewInt(0x5812631a5cf5d3ed))
...
func GetBytesOrder() [32]byte {
	order := GetBigIntOrder()

	var orderBytesRlt [32]byte
	copy(orderBytesRlt[32-len(order.Bytes()):], order.Bytes())

	ScReverse(&orderBytesRlt)

	return orderBytesRlt
}
...
func GeAdd(C *ExtendedGroupElement, A *ExtendedGroupElement, B *ExtendedGroupElement) {
	...
	var cComplete CompletedGroupElement
	B.ToCached(&bCached)
	geAdd(&cComplete, A, &bCached)
	cComplete.ToExtended(C)
}
...
// selectCachedPoint select cache point
func selectCachedPoint(t *CachedGroupElement, cached *[8]CachedGroupElement, b int32) {
	...
	t.Zero()
	for i := int32(0); i < 8; i++ {
		CachedGroupElementCMove(t, &cached[i], equal(bAbs, i+1))
	}
	FeCopy(&minusT.yPlusX, &t.yMinusX)
	FeCopy(&minusT.yMinusX, &t.yPlusX)
	FeCopy(&minusT.Z, &t.Z)
	FeNeg(&minusT.T2d, &t.T2d)
	CachedGroupElementCMove(t, &minusT, bNegative)
}
...
func GeScalarMult(h *ExtendedGroupElement, a *[32]byte, p *ExtendedGroupElement) {
	...
	var r CompletedGroupElement
	var s ProjectiveGroupElement
	var t2, t3, t4, t5, t6, t7, t8 CompletedGroupElement
	var p2, p3, p4, p5, p6, p7, p8 ExtendedGroupElement
	var pi [8]CachedGroupElement
	...
	p.Double(&t2)
	t2.ToExtended(&p2)
	p2.ToCached(&pi[2-1]) /* 2p = 2*p */

	geAdd(&t3, p, &pi[2-1])
	t3.ToExtended(&p3)
	p3.ToCached(&pi[3-1]) /* 3p = 2p+p */

	p2.Double(&t4)
	t4.ToExtended(&p4)
	p4.ToCached(&pi[4-1]) /* 4p = 2*2p */

	geAdd(&t5, p, &pi[4-1])
	t5.ToExtended(&p5)
	p5.ToCached(&pi[5-1]) /* 5p = 4p+p */

	p3.Double(&t6)
	t6.ToExtended(&p6)
	p6.ToCached(&pi[6-1]) /* 6p = 2*3p */

	geAdd(&t7, p, &pi[6-1])
	t7.ToExtended(&p7)
	p7.ToCached(&pi[7-1]) /* 7p = 6p+p */

	p4.Double(&t8)
	t8.ToExtended(&p8)
	p8.ToCached(&pi[8-1]) /* 8p = 2*4p */
	...
	var i = 63
	for ; i != 0; i-- {
		selectCachedPoint(&t, &pi, int32(e[i]))
		geAdd(&r, h, &t)
