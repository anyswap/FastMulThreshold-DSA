...
func SignInitAcceptData2(sbd *SignBrocastData,workid int,sender string,ch chan interface{}) error {
    ...
    if ok {
		...
		for _,vv := range sbd.PickHash {
		   ...
		   if err != nil {
			common.Info("===============InitAcceptData2,pick pre-sign data by key fail.===================","txhash",vv.Hash,"pickkey",vv.PickKey,"key",key,"err",err)
			res := RpcSmpcRes{Ret: "", Tip: "pick pre-sign data by key fail", Err: errors.New("pick pre-sign data by key fail")}
			ch <- res
			DtPreSign.Unlock()
			return errors.New("pick pre-sign data by key fail")
		   }
			...
			res := RpcSmpcRes{Ret: "", Tip: "delete pre-sign data from db fail", Err: errors.New("delete pre-sign data from db fail")}
			ch <- res
			DtPreSign.Unlock()
...
func Sign(raw string) (string, string, error) {
	...
	common.Info("============Sign,SendMsgToSmpcGroup===============","raw ",raw,"gid ",sig.GroupId,"key ",key)
	...
	for {
		...
		if exsit {
			...
			if ok {
			    ...
			    SendMsgToSmpcGroup(send,rsd.GroupId)
...
func GetSignStatus(key string) (string, string, error) {
	...
	if !exsit || da == nil {
		...
		return "", "smpc back-end internal error:get sign accept data fail from db when GetSignStatus", fmt.Errorf("smpc back-end internal error:get sign accept data fail from db when GetSignStatus")
	}

	ac,ok := da.(*AcceptSignData)
	if !ok {
		common.Info("=================GetSignStatus,get sign accept data error from db================","key",key)
		return "", "smpc back-end internal error:get sign accept data error from db when GetSignStatus", fmt.Errorf("smpc back-end internal error:get sign accept data error from db when GetSignStatus")
	}
...
func Sign_ed(msgprex string, save string, sku1 *big.Int, message string, cointype string, pk string, ch chan interface{}, id int) string {
	...
	_,err := h.Write(sk[32:])
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write sk fail in caling R", Err: err}
	    ch <- res
	    return ""
	}
	...
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write message fail in caling R", Err: err}
	    ch <- res
	    return ""
	}
	...
	if cherr != nil {
		...
		res := RpcSmpcRes{Ret: "", Tip: tip, Err: fmt.Errorf("get ed c21 timeout.")}
		ch <- res
		return ""
	}
	...
	if w.msg_edc21.Len() != w.NodeCnt {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get msg_edc21 fail", Err: fmt.Errorf("get all ed c21 fail.")}
		ch <- res
		return ""
	}
	...
	if cherr != nil {
		...
		res := RpcSmpcRes{Ret: "", Tip: tip, Err: fmt.Errorf("get ed zkr timeout.")}
		ch <- res
		return ""
	}
	...
	if w.msg_edzkr.Len() != w.NodeCnt {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get all msg_edzkr fail", Err: fmt.Errorf("get all ed zkr fail.")}
		ch <- res
		return ""
	}
	...
	if cherr != nil {
		...
		res := RpcSmpcRes{Ret: "", Tip: tip, Err: fmt.Errorf("get ed d21 timeout.")}
		ch <- res
		return ""
	}
	...
	if w.msg_edd21.Len() != w.NodeCnt {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get all msg_edd21 fail", Err: fmt.Errorf("get all ed d21 fail.")}
		ch <- res
		return ""
	}
	...
	for _, id := range idSign {
		...
		if !CRFlag {
			...
			res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:commitment verification fail in ed sign", Err: fmt.Errorf("Commitment(R) Not Pass.")}
			ch <- res
			return ""
		}
	}
	...
	for _, id := range idSign {
		...
		if !zkRFlag {
			...
			res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:zeroknowledge verification fail in ed sign", Err: fmt.Errorf("ZeroKnowledge Proof (R) Not Pass.")}
			ch <- res
			return ""
		}
	}
	...
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write final r fail in caling k", Err: fmt.Errorf("write final r fail in caling k")}
	    ch <- res
	    return ""
	}
	...
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write pk fail in caling k", Err: fmt.Errorf("write pk fail in caling k")}
	    ch <- res
	    return ""
	}
	...
	_,err = h.Write(([]byte(message))[:])
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write message fail in caling k", Err: fmt.Errorf("write message fail in caling k")}
	    ch <- res
	    return ""
	}
	...
	if cherr != nil {
		...
		res := RpcSmpcRes{Ret: "", Tip: tip, Err: fmt.Errorf("get ed c31 timeout.")}
		ch <- res
		return ""
	}
	...
	if w.msg_edc31.Len() != w.NodeCnt {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get msg_edc31 fail", Err: fmt.Errorf("get all ed c31 fail.")}
		ch <- res
		return ""
	}
	...
	if cherr != nil {
		...
		res := RpcSmpcRes{Ret: "", Tip: tip, Err: fmt.Errorf("get ed d31 timeout.")}
		ch <- res
		return ""
	}
	...
	if w.msg_edd31.Len() != w.NodeCnt {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get all msg_edd31 fail", Err: fmt.Errorf("get all ed d31 fail.")}
		ch <- res
		return ""
	}
	...
	for _, id := range idSign {
		...
		if !CSBFlag {
			...
			res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:commitment(CSB) not pass", Err: fmt.Errorf("Commitment(SB) Not Pass.")}
			ch <- res
			return ""
		}
	}
	...
	_,err = h.Write(FinalRBytes[:])
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write final r fail in caling k2", Err: fmt.Errorf("write final r fail in caling k2.")}
	    ch <- res
	    return ""
	}
	...
	_,err = h.Write(pkfinal[:])
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write final pk fail in caling k2", Err: fmt.Errorf("write final pk fail in caling k2.")}
	    ch <- res
	    return ""
	}
	...
	_,err = h.Write(([]byte(message))[:])
	if err != nil {
	    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:write message fail in caling k2", Err: fmt.Errorf("write message fail in caling k2.")}
	    ch <- res
	    return ""
	}
	...
	if !bytes.Equal(sBBytes2[:], sBCalBytes[:]) {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:not pass verification (CSB == SBCal)", Err: fmt.Errorf("Error: Not Pass Verification (SB = SBCal).")}
		ch <- res
		return ""
	}
	...
	if cherr != nil {
		...
		res := RpcSmpcRes{Ret: "", Tip: tip, Err: fmt.Errorf("get ed s timeout.")}
		ch <- res
		return ""
	}
	...
	if w.msg_eds.Len() != w.NodeCnt {
		...
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get msg_eds fail", Err: fmt.Errorf("get all ed s fail.")}
		ch <- res
		return ""
	}
...
func sign(wsid string,account string,pubkey string,inputcode string,unsignhash []string,keytype string,nonce string,mode string,pickhash []*PickHashKey ,ch chan interface{}) {
	...
	if keytype == "ED25519" {
	    ...
	    if chandler == nil {
		    res := RpcSmpcRes{Ret: "", Tip: "cointype is not supported", Err: GetRetErr(ErrCoinTypeNotSupported)}
		    ch <- res
		    return
	    }
	    realsmpcfrom, err := chandler.PublicKeyToAddress(pubkey)
	    if err != nil {
		    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:get smpc addr error from pubkey:" + pubkey, Err: fmt.Errorf("get smpc addr fail")}
		    ch <- res
		    return
	    }
	    amount, ok := new(big.Int).SetString("1230000", 10)
	    if !ok {
		    res := RpcSmpcRes{Ret: "", Tip: "lockout value error", Err: fmt.Errorf("lockout value error")}
		    ch <- res
		    return
	    }
	    ...
	    lockouttx, digests, buildTxErr = chandler.BuildUnsignedTransaction(realsmpcfrom, pubkey, "rwybySLAzoJcqB44HQrwyk42z7jzu3GfSS", amount, "","xrp test")
	    
	    if buildTxErr != nil || lockouttx == nil || len(digests) == 0 {
		    res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:build unsign transaction fail", Err: buildTxErr}
		    ch <- res
		    return
	    }

	    sign_ed(wsid,digests,save,sku1,smpcpub,keytype,rch)
		...
		if err != nil {
			res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:new sign transaction fail", Err: err}
			ch <- res
			return
		}
	    ...
	    if err != nil {
		res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:pubkey to smpc addr fail", Err: err}
		ch <- res
		return
	    }
	    ...
	    sign_ec(wsid,unsignhash,save,sku1,smpcpkx,smpcpky,inputcode,keytype,pickhash,rch)
	    ret, tip, cherr := GetChannelValue(waitall,rch)
...
func sign_ec(msgprex string, txhash []string, save string, sku1 *big.Int, smpcpkx *big.Int, smpcpky *big.Int, inputcode string,keytype string, pickhash []*PickHashKey,ch chan interface{}) string {
	...
	for _,v := range tmp {
		...
		sd := &SignData{MsgPrex:msgprex,Key:key,InputCodeT:inputcode,Save:save,Sku1:sku1,Txhash:vv,GroupId:w.groupid,NodeCnt:w.NodeCnt,ThresHold:w.ThresHold,SmpcFrom:w.SmpcFrom,Keytype:keytype,Cointype:"",Pkx:smpcpkx,Pky:smpcpky,PickKey:pickkey}
		...
		if err != nil {
		    ...
		    //res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error:marshal sign data error", Err: err}
		    //ch <- res
		    return 
		}
		...
		if cherr != nil {
		    ...
		    //res := RpcSmpcRes{Ret: "", Tip: "smpc back-end internal error: sign fail", Err: cherr}
		    //ch <- res
		    return 
		}
